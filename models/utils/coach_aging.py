...
from models.loss import id_loss
from models.loss import w_norm
...
from models.loss.aging_loss import AgingLoss
...
class Coach:
	def __init__(self, opts):
		...
		# Initialize network TODO 设置eval？
	...
	def __set_target_to_source(self, x, input_ages):
		return [paddle.concat((img, age * paddle.ones((1, img.shape[1], img.shape[2]))))
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				loss, loss_dict, id_logs = self.calc_loss(x, y, y_hat, latent,
														  target_ages=target_ages, # 目标age
														  input_ages=input_ages, # 输入图片的age
														  no_aging=no_aging,
														  data_type='real')
				...
				if self.global_step % self.opts.image_interval == 0 or (self.global_step < 1000 and self.global_step % 25 == 0):
					self.parse_and_log_images(id_logs, x, y, y_hat, y_recovered, title='images/train/faces')
	...
	def configure_optimizers(self):
		...
		if self.opts.optim_name == 'adam':
			...
			optimizer = Ranger(learning_rate=self.opts.learning_rate, parameters=params)
		return optimizer
	...
	def calc_loss(self, x, y, y_hat, latent, target_ages, input_ages, no_aging, data_type="real"):
		...
		if self.opts.l2_lambda_crop > 0:
			...
			loss_l2_crop = self.mse_loss(y_hat[:, :, 35:223, 32:220], y[:, :, 35:223, 32:220])
